...
function filterMovie(by, database) {
    let result = []
    let obj = {}
    for (let x = 0; x < database.length; x++) {
        if (database[x].bahasa === by) {
            result.push(database[x])
        }
    }
...
function render() {
    ...
    let abroadData = filterMovie('inggris', database)
    let localData = filterMovie('indonesia', database)

    for (let i = 0; i < abroadData.length; i++) {
        abroad.innerHTML += `<div class="card w-80 glass ">
            <figure style="height: 350px;">
                <img src="${abroadData[i].image}" alt="${abroadData[i].judul}" style="height:100%; width: 100%;" />
            </figure>
            <div class="card-body ">
    ...
    for (let i = 0; i < localData.length; i++) {
        local.innerHTML += `<div class="card w-80 glass ">
            <figure style="height: 350px;">
                <img src="${localData[i].image}" alt="${localData[i].judul}" style="height:100%; width: 100%;" />
            </figure>
            <div class="card-body ">
